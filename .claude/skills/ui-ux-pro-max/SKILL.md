...
This skill is installed inside the repo at `.claude/skills/ui-ux-pro-max/`. Run the search script from the **repo root** (paths below are repo-root-relative); if the working directory is a subfolder, prefix the path accordingly or `cd` to the repo root first:

```bash
python3 .claude/skills/ui-ux-pro-max/scripts/search.py "<query>" --domain <domain>
```

If `python3` is not found, try `python`, then `py -3`. Requires Python 3.x, standard library only — no external dependencies, no network access. If Python is missing, ask the user to install it rather than installing it yourself; fall back to `references/quick-reference.md` in the meantime.
